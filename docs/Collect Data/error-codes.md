...
| 326  | Too Many Metrics                          | The processor created more distinct metrics (counters and gauges combined) than allowed. Usually caused by deriving metric names from unbounded values — for example, embedding an address or a token symbol in the metric name. Use a fixed set of metric names and put variable values in labels instead.                   |
| 327  | Too Many Event Types                      | The processor created more distinct event log types than allowed. Usually caused by deriving event names from unbounded values. Use a fixed set of event names and put variable values in event attributes instead.                                                                                                           |
| 328  | Too Many Time Series                      | A single metric has more time series (distinct label combinations) than allowed, or the total number of time series across all metrics exceeded the limit. Avoid using unbounded values — such as block numbers, timestamps, or transaction hashes — as label values.                                                         |
| 329  | Too Many Entity Types                     | The entity schema defines more entity types (entities, interfaces, and aggregations) than allowed. Reduce the number of types defined in the GraphQL schema.                                                                                                                                                                  |
| 330  | Incompatible SDK                          | The processor is built with an SDK version that is incompatible with the runtime — for example, it emits data forms the runtime no longer supports. This is permanent and not retried; upgrade the processor SDK, rebuild, and re-upload.                                                                                      |
| 331  | Load Processor Failed                     | The processor code threw while the runtime was loading it, before any handler ran — usually in top-level initialization, for example an `eth_call` or HTTP request made while the module is imported, or a `Promise.all` that fails as soon as one of those calls rejects. Retried after one hour in case the failure was transient; check the stack trace, guard or remove the failing initialization call, and re-upload.                                    |
